...
import org.jlab.rec.ecn.ECStrip;
...
public class ECUtilities {
    ...
    public static ECStrip  createStrip(Detector geom, int sector, int superlayer, int layer, int strip){
        Layer   dLayer = geom.getSector(sector).getSuperlayer(superlayer).getLayer(layer);
        if(dLayer!=null){
            ScintillatorPaddle comp = (ScintillatorPaddle) dLayer.getComponent(strip);
            if(comp!=null){
                ECStrip ecstrip = new ECStrip(sector,superlayer*3+layer,strip);
                ecstrip.getLine().copy(comp.getLine());
                return ecstrip;
            }
        }
        System.out.println("----> ec strip create error for sector = " 
        + sector + " superlayer = " + superlayer + " layer = " + layer +
                "  strip = " + strip );
        return null;
    }
